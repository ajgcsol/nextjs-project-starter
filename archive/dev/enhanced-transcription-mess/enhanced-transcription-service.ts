...
export class EnhancedTranscriptionService {
  ...
  constructor() {
    // Initialize AWS Transcribe (don't fail entirely if it's not available)
    try {
      ...
      this.awsTranscribe = null as any;
    }
    
    // Initialize Whisper AI (optional fallback)
    try {
    ...
    // Initialize OpenAI for entity extraction (optional enhancement)
    ...
    const availableServices = [
      this.awsTranscribe && 'AWS Transcribe',
      this.whisperAI && 'Whisper AI',
      'Mux (fallback)'
    ].filter(Boolean);
    
    console.log(`🎤 Enhanced transcription service initialized with ${availableServices.length} methods:`, availableServices.join(', '));
  }
  ...
  async processVideoTranscription(options: {
    ...
    if (this.awsTranscribe) {
      ...
      console.log('⚠️ AWS Transcribe not available, skipping...');
    }
    ...
    // Final fallback to Mux (if video has been processed by Mux)
    console.log('3️⃣ Falling back to Mux transcription...');
    
    try {
      // Import here to avoid circular dependency
      const { default: MuxVideoProcessor } = await import('./mux-video-processor');
      const { VideoDB } = await import('./database');
      
      // Get video from database to check for Mux asset ID
      const video = await VideoDB.findById(options.videoId);
      
      if (video && video.mux_asset_id) {
        console.log('🎬 Found Mux asset for video:', video.mux_asset_id);
        
        const muxResult = await MuxVideoProcessor.generateCaptions(video.mux_asset_id, {
          language: options.language || 'en',
          generateVtt: true,
          generateSrt: true
        });
        
        if (muxResult.success && muxResult.transcriptText) {
          console.log('✅ Mux transcription successful');
          
          const result = await this.processTranscriptionResult({
            method: 'mux',
            text: muxResult.transcriptText,
            confidence: muxResult.confidence || 0.85,
            enableEntityExtraction: options.enableEntityExtraction,
            enableAIEnhancement: options.enableAIEnhancement,
            processingTime: Date.now() - startTime
          });
          ...
          console.log('⚠️ Mux transcription failed:', muxResult.error);
        }
      } else {
        console.log('⚠️ No Mux asset found for video, cannot use Mux fallback');
      }
    } catch (error) {
      console.log('⚠️ Mux transcription error:', error);
    }
    ...
    return {
      ...
      error: 'All transcription services failed (AWS Transcribe, Whisper AI, and Mux)',
      processingTime: Date.now() - startTime
    };
  }
